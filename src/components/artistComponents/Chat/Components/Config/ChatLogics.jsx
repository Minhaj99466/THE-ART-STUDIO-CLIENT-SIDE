export const isSameSenderMargin = (messages, m, i, userId) => {
    const currentSenderId =
      (m.sender.artist && m.sender.artist._id) || // Check sender.artist
      (m.sender.user && m.sender.user._id); // Check sender.user
  
    if (
      i < messages.length - 1 &&
      (messages[i + 1].sender.artist &&
        messages[i + 1].sender.artist._id === currentSenderId ||
      ...
      (i < messages.length - 1 &&
        ((!messages[i + 1].sender.artist ||
          ...
          (!messages[i + 1].sender.user ||
            messages[i + 1].sender.user._id !== currentSenderId)) &&
        ((m.sender.artist && m.sender.artist._id !== userId) ||
  ...
  export const isSameSender = (messages, m, i, userId) => {
    const currentSenderId =
      m.sender.artist && m.sender.artist._id
        ? m.sender.artist._id
        : m.sender.user._id;
    const nextSenderId =
      messages[i + 1] &&
      (messages[i + 1].sender.artist && messages[i + 1].sender.artist._id
        ? messages[i + 1].sender.artist._id
    ...
    export const isLastMessage = (messages, i, userId) => {
      const lastSenderId =
        messages[messages.length - 1].sender.artist &&
        messages[messages.length - 1].sender.artist._id
          ? messages[messages.length - 1].sender.artist._id
